 ...
 * 秦同学
 */
public class Node3 extends AbstractNode<String, String> {
...
//        Thread.sleep(1000L);
    ...
    public void begin() {
        ...
        throw new RuntimeException("自己抛出错误");
    }

    @Override
    public void result(boolean success, String param, WorkResult<String> workResult) {
        if (success) {
            System.out.println("callback success--" + SystemClock.now() + "----" + workResult
                    + "-threadName:" + Thread.currentThread().getName());
        } else {
            System.err.println("callback failure--" + SystemClock.now() + "----" + workResult
                    + "-threadName:" + Thread.currentThread().getName());
        }
    }
